        ...
        protected void OutputStatsToFile()
        {
            ...
            using (FileStream fs = new FileStream("commands.txt", FileMode.Create, FileAccess.Write, FileShare.Read))
            {
                using (StreamWriter sw = new StreamWriter(fs))
                {
                    ...
                    {
                        int stopindex = listBoxCommands.Items.Count - 1 - 28;
            ...
            using (FileStream fs = new FileStream("utc.txt", FileMode.Create, FileAccess.Write, FileShare.Read))
            {
                using (StreamWriter sw = new StreamWriter(fs))
                {
                    sw.WriteLine(DateTime.Now.ToString("UTC "+ @"mm:ss:ff"));
                    sw.Close();
                }
            }}
